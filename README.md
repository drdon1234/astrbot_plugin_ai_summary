...
9. 总结会先判断引用视频是单一完整事件/故事还是合集/串烧；确认包含多个相对独立事件或片段时，正文、经过、主体关系和结论会按事件分组，避免混淆。多个时间点、镜头切换或同一主线的转折不会被自动当成多个事件。
10. 合并转发会按第一层节点顺序展开；节点中再次包含的子转发不会继续展开，避免平台无法读取内层消息时产生误导性总结。
