...
DEFAULT_SUMMARY_SYSTEM_PROMPT = (
    ...
    "输出前必须先判断内容结构：引用内容是一个连续完整的事件/故事，还是包含两个及以上可以独立叙述的事件、案例、新闻、故事或片段的合集/串烧。"
    "多个时间点、镜头切换或同一事件的多个转折不等于多个事件；只有存在相对独立的主体、冲突/主题、场景或可分开叙述的主线时，才按多事件处理。"
    "如果是合集/串烧，必须按事件或片段分别组织正文、经过、主体/角色关系和结论，不能把不同事件的信息混成一条主线；如果是单一事件，保持连续叙述。"
    "自动模式下，故事讲述、电影解说或影视剧情内容应提炼人物、冲突、关键转折和结局，使用精华解说的叙述方式；不要仅因出现案件、警方、调查、死亡等剧情词就套用新闻栏目，也不要虚构新闻处置进展。用户明确选择新闻摘要时遵循其指定格式，但仍只写可确认事实。"
...
5. 如果内容可能是合集/串烧，且需要依靠画面切换、场景或人物变化区分独立片段，应启用视觉兜底帮助确认边界。
...
4. 判断内容是一个连续事件/故事，还是包含多个相对独立事件或片段的合集/串烧；按时间顺序标出独立片段的边界，但不要把普通镜头切换或同一事件的转折误判为多个事件。
5. 如果画面存在成人化/擦边/暴露/挑逗展示，只做中立安全描述，不输出露骨细节。
6. 不要识别真人身份，不要猜测不可见动机，不要把不确定信息写成事实。
7. 最后先给出“内容结构：单一事件/故事”或“内容结构：合集/串烧”；如果是合集/串烧，列出可确认的独立片段边界。
8. 再给出“可用于总结的信息密度：高/中/低”和一句局限说明。
...
9. 先判断内容结构是“单一完整事件/故事”还是“多个独立事件/片段的合集/串烧”。多个时间点、镜头切换或同一主线的转折不算多个事件；只有存在相对独立的主体、冲突/主题、场景或主线时才拆分。
10. 如果是多个事件/片段的合集/串烧，必须为每个事件分别组织正文、事件经过、主体/角色关系和结论；可以使用“事件一/事件二”或能概括该事件的小标题，禁止把不同事件的主体、时间、数据和结论混写。单一事件才使用一条连续主线。
...
3. 对故事讲述、电影解说、影视剧情等叙事内容，做精华解说：按人物、冲突、关键转折和结局（或当前进展）讲清楚，保留能串起剧情的因果，不要套用新闻栏目。
4. 单一完整事件通常输出 1-2 个自然段；信息密度很高时最多 3 个自然段。若确认是多个独立事件/片段的合集/串烧，每个事件必须单独成段或使用简短小标题，段数按事件数量展开，不能为了压缩篇幅把它们合并。
5. 不要使用固定栏目，不要输出“关键总结”“事件脉络”“主体关系”“经验启示”等章节。
6. 低信息密度、游戏、舞蹈、音乐、AIGC 展示或纯画面展示类视频，用 1-3 句概括即可。
"""
...
5. 如果确认包含多个独立新闻事件/案例，先用事件小标题分开，再在各事件下分别整理背景、经过、身份数据和处置进展；不要把所有事件共用一套主体或时间线。
6. 如果引用内容不是新闻、案件通报或突发事件，也要保持新闻摘要写法，但只做中立事实整理，不强行编造处置进展。自动模式下，故事讲述、电影解说和影视剧情不应选用本格式。
"""
...
7. 如果确认是多个独立事件/片段的合集/串烧，关键总结、内容脉络、主体关系和结论必须按事件分组；每组只写该事件的信息，禁止把不同事件拼成一条因果链。
8. AI 总结用一句有判断力的话收束视频核心结论；若是合集，可概括各事件共同主题，但不要抹掉各事件的差异。
"""
